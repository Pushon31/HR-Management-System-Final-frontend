...
export class ProjectListComponent implements OnInit {
  ...
  if (this.authService.hasRole('ROLE_ADMIN')) {
    ...
    return '/manager/projects/add';
  } else {
    return '/employee/dashboard'; // Employees can't create projects
  }
}
...
getProjectEditRoute(projectId: number): string {
  if (this.authService.hasRole('ROLE_ADMIN')) {
    return `/admin/projects/edit/${projectId}`;
  } else if (this.authService.hasRole('ROLE_MANAGER') || 
             this.authService.hasRole('ROLE_HR') || 
             this.authService.hasRole('ROLE_ACCOUNTANT')) {
    return `/manager/projects/edit/${projectId}`;
  } else {
    return `/employee/dashboard`; // Employees can't edit projects
  }
}
}